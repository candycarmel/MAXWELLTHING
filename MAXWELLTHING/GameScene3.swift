...
class GameScene3: SKScene, SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if contact.bodyA.node?.name == "ground" || contact.bodyB.node?.name == "ground" {
            ...
            diePlease = true
        } else if contact.bodyA.node?.name == "check" || contact.bodyB.node?.name == "check" {
            lastCheckpoint = ((contact.bodyA.node?.name == "check") ? contact.bodyA.node! : contact.bodyB.node!).position
        } else {
